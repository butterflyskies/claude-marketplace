...
- **Concurrency**: how will components that share state coordinate? Apply
  `principle-prefer-message-passing` — default to channels and lock-free structures
  (Rust `ArcSwap`/atomics, Go channels, JS Atomics), reaching for a mutex only to
  serialize side effects (IO, file writes), not to protect data other tasks read.
